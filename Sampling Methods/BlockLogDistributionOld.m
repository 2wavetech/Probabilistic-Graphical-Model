...
function LogBS = BlockLogDistributionOld(V, G, F, A)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
v2f = unique(cat(2, G.var2factors{V}));         % factor indices to F associated with V and its neighbors
P = struct ('var', [], 'card', [], 'val', []);  % used for iterative factor product
%**************************************************************************
% CPD P(V | X-v) is propotional to the joint distribution so there is no
% need to calculate the marginal over X-v and divide joint distribution.
%**************************************************************************
for i = 1:length(v2f)
   F(v2f(i)).val = log(F(v2f(i)).val);          % turn the ith factor into log form
   P = FactorSum(P, F(v2f(i)));                 % log(factor product) => factor sum
   varMarg = setdiff(F(v2f(i)).var, intersect(F(v2f(i)).var,V)); % variables X-v to sum out
   Evidence = [varMarg', A(varMarg)'];          % build up the evidence
   P = ObserveEvidence(P, Evidence);            % first, apply evidence - assignment of X-v
   P = FactorMarginalization(P, varMarg);       % then sum out X-v
end
validAssignments = repmat(1:d, length(V), 1);   % Y's assignments - every column has the same assignment value
LogBS = GetValueOfAssignment(P, validAssignments');  % final result
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Re-normalize to prevent underflow when you move back to probability space
% % Jiyang's note: this normalization keeps consistency with randsample().
