...
if bSwendsenWang
  ...
  for i = 1:size([u, v], 1)
      ...
      if u(i) > v(i)
          edge_factor = F(E2F{u(i), v(i)}(1));   % E2F{u(i), v(i)}(1) is the index to F
          ...
          elseif strcmp(TransName, 'MHSwendsenWang2')
              ...
              %         u             u, v
              assign = IndexToAssignment(1:length(edge_factor.val), edge_factor.card);
              q_ij = sum(edge_factor.val(find(assign(:,1) == assign(:,2))))/sum(edge_factor.val);
...
indx = mix_time+1:sampling_interval:size(all_samples, 1) ;
M = ExtractMarginalsFromSamples(G, all_samples, indx);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
