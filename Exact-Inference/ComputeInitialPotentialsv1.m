...
%**********************************************************************
% Calculate the initial potential of each clique node 
%**********************************************************************
for i = 1:N
    F = struct('var', [], 'card', [], 'val', []);
    varFactorInCliqueScope = [];
    for j = 1:length(C.factorList)                  % scan through factorList, factor product => F
        if factorAssignments(j) == i
            %*************************************************************
            % align the order of variables of factor to that of clique
            %*************************************************************
            [dummy, mapB] = ismember(C.nodes{i}, C.factorList(j).var);
            idx = find(mapB>0);
            varFactorInCliqueScope = C.nodes{i}(idx);
            factorC = transposeFactor(C.factorList(j), varFactorInCliqueScope);
