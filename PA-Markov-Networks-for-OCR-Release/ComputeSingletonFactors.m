...
% Run: factors = ComputeSingletonFactors(allWords{1}, imageModel);
%      imageModel is one of the outputs of load('PA3Models.mat')
% Output:
...
for imageIndex =1:n
    P = ComputeImageFactor (images(imageIndex).img, imageModel);
    % [factors(imageIndex).val, factors(imageIndex).var(1)] = max(P);
    factors(imageIndex).card = imageModel.K;
    factors(imageIndex).val = P;
    factors(imageIndex).var = imageIndex;
end
end
